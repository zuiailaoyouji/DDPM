...
v4 变更
────────
将语义教师从 HoVer-Net 替换为 CellViT：
  - generate_reconstructions 改为调用 CellViT 推理
  - cls_pred / conf_pred / nuc_mask_pred 来自 CellViT 输出
  - conf_pred 改为 nuclei_type_prob 的最大值（置信度代理）
  - nuc_mask  改为 nuclei_binary_map softmax[:,1]
  - cls_clean / nuc_mask_clean 仍来自 GT masks.npy

v3 变更（保留）
────────
  LR pred 行：cls_pred_lr / conf_pred_lr / nuc_mask_pred_lr

v2 变更（保留）
────────
  cls_clean / nuc_mask_clean 来自 GT masks.npy，不再对 HR 跑语义模型
"""
...
def _load_fixed_validation_batch_pannuke(
    ...
    for idx in selected_indices:
        ...
        gt_label_maps.append(sample['gt_label_map'])
        gt_nuc_masks.append(sample['gt_nuc_mask'])

    hr_t  = torch.stack(hrs).to(device)
    lr_t  = torch.stack(lrs).to(device)
    lbl_t = torch.stack(labels).to(device)
    gt_label_map_t = torch.stack(gt_label_maps).to(device)
    gt_nuc_mask_t  = torch.stack(gt_nuc_masks).to(device)
...
# ─────────────────────────────────────────────────────────────────────────────
# CellViT 推理辅助（仅用于验证可视化）
# ─────────────────────────────────────────────────────────────────────────────

def _run_cellvit_for_vis(cellvit, img_01: torch.Tensor):
    """
    img_01 : [B, 3, H, W] in [0, 1]
    返回：
      cls_label : [B, 1, H, W] float   类别索引图
      conf      : [B, 1, H, W] float   最大概率（置信度代理）
      nuc_mask  : [B, 1, H, W] float   核概率图
    """
    out      = cellvit(img_01)
    type_prob = F.softmax(out['nuclei_type_map'], dim=1)    # [B,6,H,W]
    cls_label = type_prob.argmax(dim=1, keepdim=True).float()  # [B,1,H,W]
    conf      = type_prob.max(dim=1, keepdim=True)[0]          # [B,1,H,W]
    bin_prob  = F.softmax(out['nuclei_binary_map'], dim=1)  # [B,2,H,W]
    nuc_mask  = bin_prob[:, 1:2, :, :]                         # [B,1,H,W]
    return cls_label, conf, nuc_mask
...
class ValidationSet:
    ...
    def __init__(self, val_dir, scheduler, device='cuda',
        ...
        self.gt_label_map = None
        self.gt_nuc_mask  = None
    ...
    def generate_reconstructions(
        ...
        with torch.no_grad():
            ...
            if self.gt_label_map is not None:
                cls_clean      = self.gt_label_map.float().unsqueeze(1)
                nuc_mask_clean = self.gt_nuc_mask.unsqueeze(1)
            else:
                cls_clean      = torch.zeros(B, 1, H, W, device=self.device)
                nuc_mask_clean = torch.zeros(B, 1, H, W, device=self.device)

            # ── SR pred 侧和 LR pred 侧：CellViT 推理 ────────────
            ...
            # ── CellViT(HR) 预测（监督目标，用于对比）────────────
            cls_hr      = torch.zeros(B, 1, H, W, device=self.device)
            conf_hr     = torch.zeros(B, 1, H, W, device=self.device)
            nuc_mask_hr = torch.zeros(B, 1, H, W, device=self.device)

            if loss_module is not None and hasattr(loss_module, 'cellvit') \
                    and loss_module.cellvit is not None:
                cellvit = loss_module.cellvit

                # HR 图预测（监督目标）
                try:
                    cls_hr, conf_hr, nuc_mask_hr = _run_cellvit_for_vis(
                        cellvit, self.hr,
                    )
                except Exception as e:
                    print(f"  ⚠️  CellViT HR 推理失败: {e}")

                # SR 重建图预测
                try:
                    cls_pred, conf_pred, nuc_mask_pred = _run_cellvit_for_vis(
                        cellvit, recon,
                    )
                except Exception as e:
                    print(f"  ⚠️  CellViT SR pred 推理失败: {e}")

                # LR 图预测
                try:
                    cls_pred_lr, conf_pred_lr, nuc_mask_pred_lr = _run_cellvit_for_vis(
                        cellvit, self.lr,
                    )
                except Exception as e:
                    print(f"  ⚠️  CellViT LR pred 推理失败: {e}")
...
def save_validation_debug_images(
    ...
    # CellViT(HR) 预测（监督目标）
    cls_hr_int      = _label(cls_hr)          if cls_hr           is not None else _zeros_label.copy()
    conf_hr_np      = _gray(conf_hr)           if conf_hr          is not None else _zeros_gray.copy()
    nuc_mask_hr_np  = _gray(nuc_mask_hr)       if nuc_mask_hr      is not None else _zeros_gray.copy()

    target_hw = hr_rgb.shape[1:3]

    for arr in [cls_clean_int, cls_pred_int, cls_pred_lr_int, cls_hr_int]:
    ...
    for arr in [conf_pred_np, nuc_clean_np, nuc_pred_np, conf_pred_lr_np, nuc_pred_lr_np,
                conf_hr_np, nuc_mask_hr_np]:
        if arr.shape[1:3] != target_hw:
            arr[:] = _resize_gray_np(arr, target_hw)

    # 颜色表（优先读 type_info.json，找不到则用默认色）
    ...
    overlay_hr    = _tp_overlay(hr_rgb,    cls_hr_int,      conf_hr_np,      nuc_mask_hr_np)
    ...
    rows_data = [
        ...
        (overlay_hr,      'TP overlay HR pred'),    # CellViT(HR)，监督目标
        (overlay_lr,      'TP overlay LR pred'),    # CellViT(LR)，超分前
        (overlay_pred,    'TP overlay SR pred'),    # CellViT(SR)，超分后
        (conf_hr_np,      'conf HR pred'),
        (conf_pred_lr_np, 'conf LR pred'),
        (conf_pred_np,    'conf SR pred'),
        (nuc_clean_np,    'nuc_mask GT'),
        (nuc_mask_hr_np,  'nuc_mask HR pred'),
        (nuc_pred_lr_np,  'nuc_mask LR pred'),
